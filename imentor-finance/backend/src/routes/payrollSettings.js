...
router.post('/', async (req, res) => {
  try {
    const { employee_name, visible, target_type, target_value, monthly_overrides, overachievement_rate, ika_percentage, is_ika_supplier } = req.body;
    ...
    const [record, created] = await PayrollEmployeeSetting.findOrCreate({
      ...
      defaults: { visible, target_type, target_value, monthly_overrides: monthly_overrides ?? {}, overachievement_rate: overachievement_rate ?? 10, ika_percentage: ika_percentage ?? 0, is_ika_supplier: is_ika_supplier ?? false }
    });
    if (!created) {
      ...
      if (ika_percentage !== undefined) patch.ika_percentage = ika_percentage;
      if (is_ika_supplier !== undefined) patch.is_ika_supplier = is_ika_supplier;
