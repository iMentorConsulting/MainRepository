...
export function buildResultsEmailHtml(data) {
  ...
  // Build rows — expand step/sub-rows when present; withdrawn creditors shown with indicator
  const creditorRows = creditors.flatMap((c) => {
    if (c.withdrawn) {
      return [`<tr style="background:#fff5f5;opacity:0.75;"><td style="padding:10px;border:1px solid #fecaca;font-weight:600;">${escHtml(c.creditor)} <span style="font-size:11px;color:#dc2626;background:#fee2e2;padding:2px 6px;border-radius:10px;font-weight:700;">Απεσύρθη</span></td><td style="padding:10px;border:1px solid #fecaca;text-align:center;color:#9ca3af;font-style:italic;" colspan="6">Δεν συμμετείχε στον εξωδικαστικό μηχανισμό</td></tr>`]
    }
    const hasSub = (c.subRows || []).length > 0
    const hasStep = (c.stepRows || []).length > 0
    if (hasStep) {
      const stepTag = `<span style="font-size:11px;background:#e0e7ff;color:#4338ca;padding:2px 6px;border-radius:10px;font-weight:700;">≡ step</span>`
      const parentRow = `<tr style="background:#f5f3ff;"><td style="padding:10px;border:1px solid #c7d2fe;font-weight:700;">${escHtml(c.creditor)} ${stepTag}</td>${tdR(c.actualWriteoff > 0 ? fmt(c.actualWriteoff) : '—', 'right', 'font-weight:700;')}${tdR(c.actualRemaining > 0 ? fmt(c.actualRemaining) : '—', 'right', 'font-weight:700;')}<td style="padding:10px;border:1px solid #c7d2fe;text-align:center;color:#4338ca;font-style:italic;">βλ. βήματα</td><td style="padding:10px;border:1px solid #c7d2fe;" colspan="3"></td></tr>`
      const stepRowsHtml = c.stepRows.map((s) => `<tr style="background:#eef2ff;"><td style="padding:8px 10px 8px 22px;border:1px solid #c7d2fe;color:#4338ca;font-style:italic;">↳ ${escHtml(s.label || '')}</td><td style="padding:8px 10px;border:1px solid #c7d2fe;" colspan="2"></td><td style="padding:8px 10px;border:1px solid #c7d2fe;text-align:right;font-weight:700;color:#1e40af;">${s.monthlyPay > 0 ? fmtDec2(s.monthlyPay) : '—'}</td><td style="padding:8px 10px;border:1px solid #c7d2fe;text-align:center;">${s.months ? `${s.months} μήν.` : '—'}</td><td style="padding:8px 10px;border:1px solid #c7d2fe;" colspan="2"></td></tr>`).join('')
      return [parentRow, stepRowsHtml]
    }
    if (!hasSub) {
