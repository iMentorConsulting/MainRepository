...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const item = await prisma.exodikastikosQuestionOption.update({ where: { id: params.id }, data: updateData })
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  await prisma.exodikastikosQuestionOption.delete({ where: { id: params.id } })
