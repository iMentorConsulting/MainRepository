    ...
    try {
      ...
      if (existing.accountant?.email) {
        ...
        await sendEmail({
          ...
          subject: `Ενημέρωση Υπόθεσης #${existing.caseNumber}`,
          html: renderCampaignEmailHtml({
            title: `Ενημέρωση Υπόθεσης #${existing.caseNumber}`,
            recipientName: accountant?.contactPerson || existing.accountant.officeName,
            bodyText: `Η υπόθεση #${existing.caseNumber} (${existing.business.onomasia || existing.business.afm}) ενημερώθηκε:\n\n${activityNotes.map(a => `• ${a.body}`).join('\n')}\n\nΔείτε την υπόθεση: ${process.env.APP_URL || 'https://logistis.i-mentor.gr'}/cases/${existing.id}`,
