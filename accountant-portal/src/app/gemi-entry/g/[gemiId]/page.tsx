...
export default function GemiDirectEntryPage() {
  const params = useParams()
  const gemiId = params.gemiId as string
  ...
  useEffect(() => {
    fetch(`/api/public/gemi-direct/${gemiId}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/public/gemi-direct/${gemiId}/save-phone`, {
        method: 'POST',
          ...
          {fetchError ? (
            ...
            <>
              ...
              <div style={{ background: '#eff6ff', borderRadius: '10px', padding: '14px 16px', marginBottom: '28px', borderLeft: '4px solid #2563eb' }}>
                <div style={{ fontSize: '15px', fontWeight: 600, color: '#1e40af', lineHeight: 1.4 }}>
