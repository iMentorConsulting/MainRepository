...
import { Shield, Lock, Eye, EyeOff, ChevronDown, ChevronUp, ExternalLink, Send } from 'lucide-react'
...
function LoginPageInner() {
  ...
  const [invite, setInvite] = useState({ ownerName: '', businessName: '', accountantEmail: '', accountantName: '' })
  ...
  const sendInvite = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/public/invite-accountant', {
        method: 'POST',
      ...
      if (res.ok) {
        setInviteResult({ ok: true, msg: 'Η πρόσκληση στάλθηκε επιτυχώς στον λογιστή σας!' })
        setInvite({ ownerName: '', businessName: '', accountantEmail: '', accountantName: '' })
      } else {
        setInviteResult({ ok: false, msg: data.error || 'Σφάλμα αποστολής. Δοκιμάστε ξανά.' })
      }
    } catch {
      setInviteResult({ ok: false, msg: 'Σφάλμα σύνδεσης. Δοκιμάστε ξανά.' })
    } finally {
      ...
      <div className="flex-1 flex flex-col justify-center px-6 py-12 lg:px-16 bg-white">
        <div className="w-full max-w-sm mx-auto">
          ...
          <div className="mt-8 pt-6 border-t border-slate-100">
            <p className="text-center text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">Δεν είστε λογιστής;</p>
            <div className="space-y-3">

              {/* Card 1 — monitor registration */}
              <a
                href="https://i-mentor.gr/#monitor"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-3 px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100 hover:border-slate-300 transition-colors group"
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-slate-700">Εγγραφή Επιχείρησης</p>
                  <p className="text-xs text-slate-500">Λαμβάνετε ενημερώσεις για επιδοτήσεις που αφορούν την επιχείρησή σας</p>
                </div>
                <ExternalLink size={14} className="text-slate-400 flex-shrink-0 group-hover:text-slate-600 transition-colors" />
              </a>

              {/* Card 2 — invite accountant */}
              <div className="rounded-xl border border-slate-200 overflow-hidden">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-700">Πρόσκληση Λογιστή</p>
                    <p className="text-xs text-slate-500">Στείλτε πρόσκληση στον λογιστή σας να εγγραφεί</p>
                  </div>
                  {showInvite ? <ChevronUp size={15} className="text-slate-400 flex-shrink-0" /> : <ChevronDown size={15} className="text-slate-400 flex-shrink-0" />}
                </button>

                {showInvite && (
                  <form onSubmit={sendInvite} className="px-4 pb-4 pt-3 bg-white border-t border-slate-100 space-y-3">
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-xs font-medium text-slate-600 mb-1">Το όνομά σας *</label>
                        <input
                          type="text"
                          value={invite.ownerName}
                          onChange={e => setInvite(p => ({ ...p, ownerName: e.target.value }))}
                          placeholder="π.χ. Γιώργος Παπαδόπουλος"
                          required
                          className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-slate-600 mb-1">Επωνυμία επιχείρησης *</label>
                        <input
                          type="text"
                          value={invite.businessName}
                          onChange={e => setInvite(p => ({ ...p, businessName: e.target.value }))}
                          placeholder="π.χ. Παπαδόπουλος ΙΚΕ"
                          required
                          className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                        />
                      </div>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-slate-600 mb-1">Email λογιστή *</label>
                      <input
                        type="email"
                        value={invite.accountantEmail}
                        onChange={e => setInvite(p => ({ ...p, accountantEmail: e.target.value }))}
                    ...
                    <div>
                      <label className="block text-xs font-medium text-slate-600 mb-1">Όνομα λογιστή <span className="text-slate-400">(προαιρετικό)</span></label>
                      <input
                        type="text"
                        value={invite.accountantName}
                        onChange={e => setInvite(p => ({ ...p, accountantName: e.target.value }))}
                        placeholder="π.χ. Μαρία Κωστάκη"
                        className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                      />
                    </div>
                    {inviteResult && (
                      <div className={`px-3 py-2 rounded-lg text-xs ${inviteResult.ok ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-red-50 text-red-700 border border-red-100'}`}>
                        {inviteResult.msg}
                      </div>
                    )}
                    <button
                      ...
                      className="w-full py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
                    >
                      {inviteSending ? 'Αποστολή...' : <><Send size={13} /> Αποστολή Πρόσκλησης</>}
                    </button>
                  </form>
                )}
              </div>
