...
import { getOccupancy, getByChannel, getFinancial } from '../api'
import {
...
const CHANNEL_COLORS_PIE = {
  booking: '#003580', airbnb: '#FF5A5F', direct: '#10B981', other: '#9CA3AF',
}
const CH_LABELS = { booking: 'Booking.com', airbnb: 'Airbnb', direct: 'Απευθείας', other: 'Άλλο' }
...
export default function Reports() {
  ...
  useEffect(() => { load() }, [tab, from, to, finGroup])
      ...
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
        {tabs.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-colors ${tab === t.id ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
          >
            {t.label}
          </button>
        ))}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-xs text-gray-500 mb-1">Μ.Ο. Πληρότητας</p>
              <p className="text-2xl font-bold text-blue-700">{occData.summary.avg_occupancy_rate}%</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-xs text-gray-500 mb-1">Μονάδες</p>
              <p className="text-2xl font-bold text-gray-700">{occData.summary.total_units}</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-xs text-gray-500 mb-1">Συνολικά Έσοδα</p>
              <p className="text-2xl font-bold text-green-700">{formatEur(occData.summary.total_revenue)}</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-xs text-gray-500 mb-1">Καθαρά Έσοδα</p>
              <p className="text-2xl font-bold text-emerald-700">{formatEur(occData.summary.total_net_revenue)}</p>
            </div>
          </div>
                  ...
                  {occData.units.map((u) => (
                    <tr key={u.unit_id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-right">
                        <span className={`font-semibold ${u.occupancy_rate >= 70 ? 'text-green-600' : u.occupancy_rate >= 40 ? 'text-amber-600' : 'text-red-500'}`}>
                          {u.occupancy_rate}%
                        </span>
                      </td>
      ...
      {!loading && tab === 'channel' && chData && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-xs text-gray-500 mb-1">Σύνολο Κρατήσεων</p>
              <p className="text-2xl font-bold text-gray-700">{chData.total_bookings}</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-xs text-gray-500 mb-1">Συνολικά Έσοδα</p>
              <p className="text-2xl font-bold text-green-700">{formatEur(chData.total_revenue)}</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-xs text-gray-500 mb-1">Καθαρά Έσοδα</p>
              <p className="text-2xl font-bold text-emerald-700">{formatEur(chData.total_net_revenue)}</p>
            </div>
          </div>
                ...
                <PieChart>
                  <Pie data={chData.channels} dataKey="bookings_count" nameKey="channel" cx="50%" cy="50%" outerRadius={80} label={({ channel, percent }) => `${CH_LABELS[channel] || channel} ${(percent * 100).toFixed(0)}%`}>
                    {chData.channels.map((c) => (
                      <Cell key={c.channel} fill={CHANNEL_COLORS_PIE[c.channel] || '#9CA3AF'} />
                    ))}
                  </Pie>
