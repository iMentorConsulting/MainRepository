...
export default function WidgetPage() {
  ...
  useEffect(() => {
    if (selected.checkIn && selected.checkOut) {
      api.get(`/widget/price/${token}`, { params: { check_in: selected.checkIn, check_out: selected.checkOut } })
        .then(r => setPriceInfo(r.data))
        .catch(() => setPriceInfo(null))
    } else {
      setPriceInfo(null)
    }
  }, [selected.checkIn, selected.checkOut, token])
      ...
      <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-6">
            ...
            {selected.checkIn && selected.checkOut ? (
              <div className="mt-5 p-4 rounded-xl border-2"
                    ...
                    {priceInfo ? (
                      <div className="mt-1 space-y-0.5">
                        <p className="text-gray-500">
                          {priceInfo.nights} νύχτες · €{priceInfo.price_per_night}/νύχτα
                        </p>
                        {priceInfo.discounts && priceInfo.discounts.length > 0 && (
                          <div className="space-y-0.5">
                            {priceInfo.discounts.map(d => (
                              <p key={d.id} className="text-green-600 font-medium text-xs">
                                🏷️ {d.name} −€{d.savings.toFixed(2)}
                              </p>
                            ))}
                          </div>
                        )}
                      ...
                      <p className="text-gray-400 text-xs">Υπολογισμός τιμής…</p>
                    )}
                  </div>
                    ...
                    {priceInfo && (
                      <div>
                        {priceInfo.total_savings > 0 && (
                          <p className="text-xs text-gray-400 line-through">€{priceInfo.subtotal.toFixed(2)}</p>
                        )}
                        <p className="text-lg font-bold" style={{ color }}>€{priceInfo.final_price.toFixed(2)}</p>
                        {priceInfo.total_savings > 0 && (
                          <p className="text-xs text-green-600 font-medium">−€{priceInfo.total_savings.toFixed(2)}</p>
                        )}
                      </div>
                    )}
                  </div>
                ...
                <button
                  className="mt-3 w-full text-white text-sm font-semibold px-5 py-2 rounded-xl"
        ...
        {step === 'form' && (
          <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-6">
            ...
            <div className="text-xs text-gray-500 mb-4 p-3 rounded-lg bg-gray-50 space-y-1">
              <p>📅 {new Date(selected.checkIn+'T00:00').toLocaleDateString('el-GR')} →{' '}
              {new Date(selected.checkOut+'T00:00').toLocaleDateString('el-GR')} · {nights} νύχτες</p>
              {priceInfo && priceInfo.discounts && priceInfo.discounts.length > 0 && (
                <div>
                  {priceInfo.discounts.map(d => (
                    <p key={d.id} className="text-green-600 font-medium">🏷️ {d.name} −€{d.savings.toFixed(2)}</p>
                  ))}
                  <p className="font-semibold text-gray-700 mt-0.5">Τελική τιμή: €{priceInfo.final_price.toFixed(2)}</p>
                </div>
              )}
              {priceInfo && priceInfo.discounts.length === 0 && priceInfo.final_price > 0 && (
                <p className="font-semibold text-gray-700">Εκτιμώμενη τιμή: €{priceInfo.final_price.toFixed(2)}</p>
              )}
            </div>
